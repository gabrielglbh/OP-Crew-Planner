...
class Migrations {
  ...
  static Future<void> version36to37(Database db) async {
    ...
    List<String> oldIds = [
      "3381", "3370", "3333", "3334", "3347", "3348", "3349", "3350",
      "3360", "3361", "3371", "3372", "3373", "3374", "3382", "3383"
    ];
    ...
    List<String> newIds = [
      "3330", "3331", "4986", "4987", "4988", "4989", "4990", "4991",
      "4992", "4993", "4994", "4995", "4996", "4997", "4998", "4999"
    ];

    for (int x = 0; x < oldIds.length; x++) {
      await db.update(Data.unitTable, {
        Data.unitId: newIds[x]
      }, where: "${Data.unitId}=? AND ${Data.unitName}=?", whereArgs: [oldIds[x], oldNamesIds[x]]);
    }
  }

  static Future<void> version35to36(Database db) async {
    await db.rawQuery("ALTER TABLE ${Data.dataTable} ADD COLUMN ${Data.art} TEXT");
    await db.update(Data.unitTable, {
      Data.unitUrl: "https://firebasestorage.googleapis.com/v0/b/optc-teams-96a76.appspot.com/o/character_11400_t1.png?alt=media&token=2550aac4-d906-400f-8630-a99945d1cf15"
    }, where: "${Data.unitId}=?", whereArgs: ["3156"]);

    // Contrast with the names too in order to not overlap the actual units with the oldIds
    List<String> oldIds = ["3351", "3352", "3353", "3354", "3356", "3357", "3358",
      "3359", "3366", "3367", "3368", "3375", "3376", "3380", "3339", "3340"];
    List<String> oldNamesIds = [
      "Emporio Ivankov [Neo]",
    ...
    List<String> newIds = ["3314", "3315", "3316", "3317", "3321", "3322", "3323",
      "3324", "3318", "3319", "3320", "3325", "3326", "3327", "3312", "3313"];
  ...
  static Future<void> version34to35(Database db) async {
    ...
    List<Unit> units = List.generate(u.length, (i) {
      return Unit(id: u[i][Data.unitId],
    ...
    for (int t = 0; t < units.length; t++) {
      ...
      await db.update(Data.unitTable, {Data.unitUrl: img}, where: "${Data.unitId}=?", whereArgs: [id]);
    }

    await db.rawQuery("ALTER TABLE ${Data.shipTable} ADD COLUMN ${Data.shipUrl} TEXT");
    List<Map<String, dynamic>> s = await db.query(Data.shipTable);
    List<Ship> ship = List.generate(s.length, (i) {
      return Ship(id: s[i][Data.shipId], name: s[i][Data.shipName], url: s[i][Data.shipUrl]);
    });
    for (int t = 0; t < ship.length; t++) {
      String id = ship[t].id;
      String img = ship[t].getUrlFromShipImg();
      await db.update(Data.shipTable, {Data.shipUrl: img}, where: "${Data.shipId}=?", whereArgs: [id]);
    }
  }
}