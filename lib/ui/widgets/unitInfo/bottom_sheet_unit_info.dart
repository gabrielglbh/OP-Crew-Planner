...
class _AdditionalUnitInfoState extends State<AdditionalUnitInfo> {
  ...
  Future<void> _getAdditionalInfoOnUnit() async {
    ...
    await UnitInfoQueries.instance
        .getUnitInfoFromDatabase(widget.uid)
        .then((info) async {
      if (info.isEqualTo(UnitInfo.empty())) {
        await BackUpRecords.instance
            .getUnitAdditionalInfo(widget.uid)
            .then((info) async {
              if (info != null) {
                setState(() {
                  _loading = false;
                  if (info.swap != null) _formatDualUnit(info);
                  if (info.vsCondition != null && info.vsSpecial != null) {
                    _formatVSUnit(info);
                  }
  ...
  _formatDualUnit(UnitInfo info) {
    if (info.captain != null) {
      info.captain =
          info.captain?.replaceFirst(_escapedCharDual2, _parsedCharDual2);
      info.captain =
          info.captain?.replaceFirst(_escapedCharDualC, _parsedCharDualC);
    }
    if (info.llbCaptain != null) {
      info.llbCaptain =
          info.llbCaptain?.replaceFirst(_escapedCharDual2, _parsedCharDual2);
      info.llbCaptain =
          info.llbCaptain?.replaceFirst(_escapedCharDualC, _parsedCharDualC);
    }
  }

  _formatVSUnit(UnitInfo info) {
    if (info.captain != null) {
      info.captain = info.captain?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.llbCaptain != null) {
      info.llbCaptain =
          info.llbCaptain?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.special != null) {
      info.special = info.special?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.llbSpecial != null) {
      info.llbSpecial =
          info.llbSpecial?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.festAbility != null) {
      info.festAbility =
          info.festAbility?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.festSpecial != null) {
      info.festSpecial =
          info.festSpecial?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.festResistance != null) {
      info.festResistance =
          info.festResistance?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.llbFestAbility != null) {
      info.llbFestAbility =
          info.llbFestAbility?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.llbFestSpecial != null) {
      info.llbFestSpecial =
          info.llbFestSpecial?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.llbFestResistance != null) {
      info.llbFestResistance =
          info.llbFestResistance?.replaceAll(_escapedCharVS, _parsedCharVS);
    }
    if (info.vsSpecial != null) {
      info.vsSpecial =
  ...
  _formatSwapSuper(UnitInfo info) {
    if (info.swap != null) {
      info.swap = info.swap?.replaceAll(_escapedCharSwap, _parsedCharSwap);
    }
  }
            ...
            return _body(
              ...
              child: Center(
                child: CircularProgressIndicator(
                    value: null, strokeWidth: 5, color: Colors.orange[400]!),
              ),
            )));
          } else {
            if (!_information.isEqualTo(UnitInfo.empty())) {
              ...
              return _body(
                  data: Expanded(
                      child: SizedBox(
                height: MediaQuery.of(context).size.height / ratioOfHeight,
                child: Center(child: Text("dataNotAvailable".tr())),
              )));
            }
          }
        });
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          ...
          data,
          Visibility(visible: _bannerIsLoaded, child: AdManager.showBanner())
        ],
      ),
    );
  }
  ...
  UnitInfoElevatedButton _downloadData() {
    return UnitInfoElevatedButton(
        onPressed: () async {
          if (!_information.isEqualTo(UnitInfo.empty())) {
            if (!_offline) {
              ...
              unit.downloaded = 1;
              await UnitInfoQueries.instance
                  .insertUnitInfoIntoDatabase(_information, unit)
                  .then((success) async {
                if (success) {
                  await UpdateQueries.instance
                      .registerAnalyticsEvent(AnalyticsEvents.downloadUnitData);
        ...
        child: Icon(_offline ? Icons.check : Icons.download_rounded,
            color: _offline ? Colors.white : null));
  }
                  ...
                  return UIAlert(
                      ...
                      content: FittedBox(
                        child: SizedBox(
                            width: 640,
                            height: 512,
                            child: UI.placeholderImageWhileLoading(art,
                                fullArt: true)),
                      ));
                });
          }
