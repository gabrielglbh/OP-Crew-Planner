...
class BuildRumbleTeamPage extends StatefulWidget {
  ...
  final bool update;
  BuildRumbleTeamPage({required this.toBeUpdatedTeam, required this.update});
        ...
        return Draggable(
          child: GestureDetector(
            ...
            child: Container(
              ...
              padding: index > 4 ? null : EdgeInsets.all(2),
    ...
    if (widget.update) {
      ...
      if (name.trim().isNotEmpty && name.length >= 4 && name.length <= 30) {
        ...
        if (newTeam.compare(widget.toBeUpdatedTeam)) {
          ...
          if (_compareUnitsWithEmpty()) UI.showSnackBar(context, "errEmptyPvP".tr());
          else {
