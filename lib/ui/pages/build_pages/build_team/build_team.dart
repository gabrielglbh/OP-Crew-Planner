...
class _BuildTeamPageState extends State<BuildTeamPage>
  ...
  AppBar _appBar() {
    return AppBar(
      title: Text(
          widget.update ? "titleUpdateTeamPage".tr() : "titleTeamPage".tr()),
                            ...
                            children: [
                              CustomSearchBar(
                                ...
                                onQuery: (query, type) => context
                                    .read<BuildBloc>()
                                  ..add(
                                      BuildEventSearching(false, query, type)),
                              ),
      ...
      builder: (context, List<int?> data, b) {
        return Draggable(
          child: GestureDetector(
              ...
              onLongPress: () {
                if (unit.id != "noimage") _removeUnitFromTeam(index);
              },
              child: Stack(
                children: [
                  Positioned(
                  ...
                  Positioned(
                    bottom:
                        (MediaQuery.of(context).size.width / 4.5) - _imgSize,
                    right: index % 2 == 0 ? null : 0,
                    child: Visibility(
                      visible:
                          !_showSupports && _supports[index].id != "noimage",
                      child: SizedBox(
                        width: _imgSupportSize,
                        height: _imgSupportSize,
                        child: FittedBox(
                            child: UI.placeholderImageWhileLoadingUnit(
                                _supports[index])),
                      ),
                    ),
                  )
  ...
  InkWell _recentUnit(int index) {
    ...
    return InkWell(
        onTap: () {
          _loseFocusOnFields();
          _onSelectedTeammate(unit.id, _showSupports ? _supports : _units,
              fromRecent: true);
        },
        child: Padding(
                    ...
                    children: [
                      ...
                      SizedBox(
                          ...
                          child: InkWell(
                            onTap: () {
                              setState(() {
                                if (_skillLevels[pos] < _maxLevelSkills[pos]) {
                                  _skillLevels[pos]++;
                                }
                              });
                            },
          ...
          return UIAlert(
            ...
            onAccepted: () async {
              await UpdateQueries.instance
                  .registerAnalyticsEvent(AnalyticsEvents.resetTeam);
              setState(() {
  ...
  _validateAndInsertTeam() {
    ...
    if (widget.update) {
      ...
      if (name.trim().isNotEmpty && name.length >= 4 && name.length <= 120) {
        ...
        if (!_units[0].id.contains("noimage") &&
            !_units[1].id.contains("noimage")) {
